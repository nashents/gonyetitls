                    ...
                    <div class="panel">
                        <div class="panel-heading">
                            ...
                            <div class="panel-title">
                                ...
                                @if ($this->unpostedDebitNotesCount > 0)
                                    <a href="#" wire:click.prevent="bulkPostToLedger()" wire:loading.attr="disabled" onclick="return confirm('Post {{ $this->unpostedDebitNotesCount }} approved debit note(s) to the general ledger?')" class="btn btn-default border-danger btn-rounded btn-wide" title="Post every approved, unposted debit note to the ledger"><i class="fa fa-book"></i> Bulk Post to Ledger ({{ $this->unpostedDebitNotesCount }})</a>
                                @endif
                            </div>
                        </div>
                            ...
                            <table id="debit_notesTable" class="table table-striped table-bordered table-sm table-responsive" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                  ...
                                  <tr>
                                    ...
                                    <td>
                                        @if ($debit_note->journal_entry)
                                            <span class="badge bg-success" title="Journal {{ $debit_note->journal_entry->journal_number }}">Posted</span>
                                        @elseif ($debit_note->authorization == 'approved')
                                            <span class="badge bg-danger">Not Posted</span>
                                            <br>
                                            <a href="#" wire:click.prevent="postToLedger({{ $debit_note->id }})" wire:loading.attr="disabled" style="color:#d9534f" title="Post to General Ledger"><i class="fa fa-book"></i> Post to Ledger</a>
                                        @else
