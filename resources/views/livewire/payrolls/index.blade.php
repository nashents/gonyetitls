<div>
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body p-20"style="overflow-x:auto; width:100%; height:100%;">
                                ...
                                <div class="form-group">
                                    <input type="text" wire:model.debounce.300ms="search" class="form-control" placeholder="Search payrolls...">
                                </div>
                            </div>
                            <table class="table table-striped table-bordered table-sm table-responsive" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                  ...
                                  <tr>
                                    ...
                                    <td>
                                        <span class="badge bg-{{($payroll->authorization == 'approved') ? 'success' : (($payroll->authorization == 'rejected') ? 'danger' : 'warning') }}">{{($payroll->authorization == 'approved') ? 'approved' : (($payroll->authorization == 'rejected') ? 'rejected' : 'pending') }}</span>
                                          @if($payroll->authorization_date)
                                        <br><small><strong style="background-color: orange">AuthorizedOn: {{ $payroll->authorization_date }}</strong></small>
                                        @endif
                                        @if($payroll->authorized_by_id)
                                            <br><small><strong style="background-color: orange">AuthorizedBy: {{ $this->getAuthorizer($payroll->authorized_by_id) }}</strong></small>
                                        @endif
                                        @if($payroll->comments)
                                            <br><small><strong style="background-color: orange">Comments: {{ $payroll->comments }}</strong></small>
                                        @endif
                                    </td>
                                    <td class="w-10 line-height-35 table-dropdown">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                ...
                                                {{-- @if ($payroll->authorization == "pending") --}}
                                                    <li><a href="#"  wire:click="edit({{$payroll->id}})" ><i class="fa fa-edit color-success"></i> Edit</a></li>
                                                    <li><a href="#" wire:click="delete({{$payroll->id}})" ><i class="fa fa-trash color-danger"></i>Delete</a></li>
                                                {{-- @endif --}}
                                            </ul>
                                        </div>
    ...
    <div wire:ignore.self data-backdrop="static" data-keyboard="false" class="modal fade" id="payrollDeleteModal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content bg-danger">
                <div class="modal-body">
                <center> <strong>Are you sure you want to delete this {{$selected_payroll?->month}} {{$selected_payroll?->year}} Payroll?</strong> </center> 
                </div>
