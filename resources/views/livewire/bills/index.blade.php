                                  ...
                                  <tr wire:key="bill-row-{{ $bill->id }}">
                                    ...
                                    <td>
                                        @if ($bill->transporter)
                                            ...
                                            @if ($bill->trip)
                                                <a href="{{ route('trips.show', $bill->trip->id) }}" style="color: blue" target="_blank">{{ $bill->trip->trip_number }}</a>
                                            @endif
                                        @elseif($bill->vendor)
                                            Vendor | <a href="{{ route('vendors.show',$bill->vendor->id) }}" style="color: blue" target="_blank">{{ $bill->vendor ? $bill->vendor->name : "" }}</a>
                                            @if ($bill->trip)
                                                <br>
                                                Trip | <a href="{{ route('trips.show', $bill->trip->id) }}" style="color: blue" target="_blank">{{ $bill->trip->trip_number }}</a>
                                            @endif
                                            @if ($bill->horse)
                                                <br>
                                                Horse | <a href="{{route('horses.show', $bill->horse->id)}}" style="color: blue" target="_blank">{{ $bill->horse ? $bill->horse->identifier_label : "" }} {{$bill->horse->horse_make ? $bill->horse->horse_make->name : ""}} {{$bill->horse->horse_model ? $bill->horse->horse_model->name : ""}}</a> 
                                            @elseif ($bill->vehicle)
                                                <br>
                                                Vehicle | <a href="{{route('vehicles.show', $bill->vehicle->id)}}" style="color: blue" target="_blank">{{ $bill->vehicle ? $bill->vehicle->identifier_label : "" }} {{$bill->vehicle->vehicle_make ? $bill->vehicle->vehicle_make->name : ""}} {{$bill->vehicle->vehicle_model ? $bill->vehicle->vehicle_model->name : ""}}</a> 
                                            @elseif ($bill->trailer)
                                                <br>
                                                Trailer | <a href="{{route('trailers.show', $bill->trailer->id)}}" style="color: blue" target="_blank">{{ $bill->trailer ? $bill->trailer->identifier_label : "" }} {{$bill->trailer->make}} {{$bill->trailer->model}}</a> 
                                            @elseif ($bill->driver)
                                                <br>
                                    ...
                                    <td>
                                        @if ($bill->total)
                                             ...
                                             @if ($bill->currency_id && $company && (int) $bill->currency_id !== (int) $company->currency_id)
                                                <hr class="my-1">
                                                <small>
                                                    {{ $company->currency?->name }} {{ $company->currency?->symbol }}
                                                    {{ number_format(
                                                            (float) (is_numeric($bill->exchange_amount)
                                                                ? $bill->exchange_amount
                                                                : preg_replace('/[^\d\.\-]/', '', (string) ($bill->exchange_amount ?? 0))
                                                            ),
                                                            2
                                                        ) }}
                                                    <br>
                                                    <strong>Rate:</strong>
                                                    {{ is_numeric($bill->exchange_rate)
                                                        ? number_format((float) $bill->exchange_rate, 4)
                                                        : ($bill->exchange_rate ?: '-') }}
                                                </small>
                                            @endif
                                        @endif
