<div>
                                    ...
                                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12" style="float: right">
                                        <a class="dashboard-stat-2 bg-primary" href="#">
                                            <div class="stat-content">
                                                <span class="name">{{Auth::user()->employee->allocation ? Auth::user()->employee->allocation->latest()->first()->balance : "No fuel allocation found"}}</span>
                                            </div>
                ...
                <div class="modal-body">
                    <div class="form-group">
                       ...
                       <select wire:model.debounce.300ms="request_type" class="form-control" required>
                        ...
                        @error('request_type') <span class="error" style="color:red">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="any" min="0" max="{{Auth::user()->employee->allocation ? Auth::user()->employee->allocation->latest()->first()->balance : "No fuel allocation found"}}"  class="form-control" wire:model.debounce.300ms="quantity" placeholder="Enter quantity below or equal to {{Auth::user()->employee->allocation ? Auth::user()->employee->allocation->latest()->first()->balance : "No fuel allocation found"}} " required>
                        @error('quantity') <span class="error" style="color:red">{{ $message }}</span> @enderror
                    </div>
                    <div class="form-group">
                        <label for="name">Date</label>
                        <input type="date" step="any" min="0"  class="form-control" wire:model.debounce.300ms="date" placeholder="Enter Date" required >
    ...
    <div wire:ignore.self data-backdrop="static" data-keyboard="false" class="modal" id="fuel_requestEditModal" tabindex="-1" role="dialog" aria-labelledby="modal4Label" data-backdrop-color="blue">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="modal4Label"><i class="fa fa-building-o"></i> Edit fuel_request <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button></h4>
                </div>
