...
class DriversPerformanceExport implements  FromQuery,
ShouldAutoSize,
WithMapping,
WithHeadings,
WithEvents,
...

{
    ...
    */
    ...
    public function map($selected_driver): array{

            $driver = Driver::find($selected_driver->driver_id);
            $name = $driver->employee ? $driver->employee->name : "";
            $surname = $driver->employee ? $driver->employee->surname : "";
            $driver_details = $name." ".$surname;
            $transporter = $driver->transporter ? $driver->transporter->name : "";

            $total_kilometers = "";
            if ($selected_driver->total_kilometers) {
                $total_kilometers = $selected_driver->total_kilometers;
            }else{
                 $total_kilometers = $this->calculateShiftsDistance($selected_driver->driver_id);
            }
            $total_hours = "";
            if ($selected_driver->total_hours) {
                $total_hours = $selected_driver->total_hours;
            }else{
                $total_hours = $this->calculateShiftsHours($selected_driver->driver_id);
            }
            $total_fuel_quantity = "";
            if ($selected_driver->total_fuel_quantity) {
                $total_fuel_quantity = $selected_driver->total_fuel_quantity;
            }else{
                $total_fuel_quantity = $this->calculateShiftsFuel($selected_driver->driver_id);
            }

            $tonnage_loss = $this->calculateTonnageLosses($selected_driver);
            $volume_loss = $this->calculateVolumeLosses($selected_driver);

            $fuel_consumption_mileage = "";

            if ($selected_driver->avg_fuel_consumption_mileage) {
                $fuel_consumption_mileage = $selected_driver->avg_fuel_consumption_mileage;
            }else{
                $fuel_consumption_mileage = $this->calculateFuelConsumptionMileage($selected_driver->driver_id);
            }
           
            $fuel_consumption_hours = "";

            if ($selected_driver->avg_fuel_consumption_hours) {
                $fuel_consumption_hours = $selected_driver->avg_fuel_consumption_hours;
            }else{
                $fuel_consumption_hours = $this->calculateFuelConsumptionHours($selected_driver->driver_id);
            }
         
                return   [
                    $transporter,
                    $driver_details,
                    $selected_driver->total_trips ,
                    ...
                    $fuel_consumption_mileage,
                    $fuel_consumption_hours,
                    $selected_driver->total_volume,
                    $selected_driver->total_volume_loss,
                    $volume_loss,
                    $selected_driver->total_tonnage,
                    $selected_driver->total_tonnage_loss,
                    $tonnage_loss,
                     ];
