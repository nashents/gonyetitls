    ...
    public function render()
    {
        if (isset($this->search)) {
            return view('livewire.inventory-products.index',[
                'products' => Product::query()->with('brand','category','category_value')->where('department','inventory')->where('status',1)
            ...
            return view('livewire.inventory-products.index',[
                'products' => Product::where('department','inventory')->where('status',1)->orderBy('name','asc')->paginate(10)
            ]);
        }
