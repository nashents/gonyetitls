    ...
    public function render()
    {
        if (isset($this->from) && isset($this->to)) {
            if (isset($this->search)) {
                return view('livewire.retreads.rejected',[
                    'retreads' => Retread::query()->with('currency','vendor','account','retread_tyres')->where('authorization','rejected')->whereBetween($this->retread_filter,[$this->from, $this->to] )
                ...
                return view('livewire.retreads.rejected',[
                    'retreads' => Retread::query()->with('currency','vendor','account','retread_tyres')->where('authorization','rejected')->whereBetween($this->retread_filter,[$this->from, $this->to] )->orderBy('retread_number','desc')->paginate(10),
                    'retread_filter' => $this->retread_filter,
                ]);
            }
        ...
        elseif (isset($this->search)) {
            ...
            return view('livewire.retreads.rejected',[
                ...
                'retreads' => Retread::query()->with('currency','vendor','account','retread_tyres')->where('authorization','rejected')
                ->where('retread_number','like', '%'.$this->search.'%')
                ->orWhere('status','like', '%'.$this->search.'%')
                ->orWhere('date','like', '%'.$this->search.'%')
                ->orWhereHas('currency', function ($query) {
            ...
            return view('livewire.retreads.rejected',[
                'retreads' => Retread::query()->with('currency','vendor','account','retread_tyres')->where('authorization','rejected')->whereMonth('created_at', date('m'))
